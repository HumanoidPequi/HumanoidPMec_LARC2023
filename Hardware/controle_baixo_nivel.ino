...
const uint8_t base_servo[DXL_ID_CNT] = {180, 180};
...
void servo_cb(const std_msgs::Int16MultiArray& cmd_msg) {
  ...
  for (int i = 0; i < DXL_ID_CNT; i++) {
    dxl.setGoalPosition(DXL_ID_LIST[i], base_servo[i] + cmd_msg.data[i], UNIT_DEGREE);
    delay(0.1);
  }
}
...
void setup() {
  ...
  for (int i = 0; i < DXL_ID_CNT; i++) {
    dxl.setGoalPosition(DXL_ID_LIST[i], base_servo[i], UNIT_DEGREE);
    delay(0.1);
  }
}
