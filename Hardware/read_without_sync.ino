...
const int DXL_DIR_PIN = 9; // DYNAMIXEL Shield DIR PIN
...
int16_t goal_position[1] = {1500};
uint8_t goal_position_index = 0;

void feedback_cb(const std_msgs::Int16MultiArray& cmd_msg){
  int16_t feedback_angle = cmd_msg.data;
  goal_position[1] = feedback_angle;
}

ros::Subscriber<std_msgs::Int16MultiArray> sub("feedback",feedback_cb);
...
void loop() {
  ...
  for (i = 0; i < DXL_ID_CNT; i++) {
    sw_data[i].goal_position = goal_position[0];
  }
