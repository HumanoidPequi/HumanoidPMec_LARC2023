...
//uint8_t head_initial[DXL_ID_CNT] = {180, 180};
  uint8_t left_leg[DXL_ID_CNT] = {180, 88, 180, 200, 202, 72, 180, 180, 180, 180, 180, 180};
  uint8_t left_leg_initial[DXL_ID_CNT] = {180, 88, 180, 200, 202, 72, 180, 180, 180, 180, 180, 180};
...
void servo_cb(const std_msgs::Int16MultiArray& cmd_msg) {
  ...
  for (int i = 0; i < DXL_ID_CNT; i++) {
    left_leg[i] = left_leg_initial[i] + cmd_msg.data[i];
...
void set_position(){
  for (int i = 0; i < DXL_ID_CNT_LEGS; i++) {
    dxl.setGoalPosition(ID_LEFT_LEG[i], left_leg[i], UNIT_DEGREE);
    delay(0.001);
  }
}
