...
const int32_t rHipRoll  =   2048-30; //2
...
//braço direito
const int32_t rSholderPitch = 3299+200;  //16

const int32_t rSholderRoll =  2204-90;  //17 BRAÇO RETO
//const int32_t rSholderRoll =  2204 + 900;  //17 BRACO ESTICADO

const int32_t rElbowPitch  =  2048;  //18

//braço esquerdo
const int32_t lSholderPitch = 2048-200;   //13

const int32_t lSholderRoll =  1991+30;   //14 BRAÇO RETO
...
std_msgs::String msg;
ros::Publisher deb("marta/position", &msg);
char str[100];
...
bool joint_reversed[MARTA_QTD] = {true, true, true, true, false, true, 
...
unsigned int minPos_arm[QTD_ARMS] = {lSholderPitch-1734, lSholderRoll-1934, lElbowPitch, rSholderPitch-1934, rSholderRoll-0, rElbowPitch};
unsigned int maxPos_arm[QTD_ARMS] = {lSholderPitch + 1934, lSholderRoll + 0, lElbowPitch ,rSholderPitch + 1934, rSholderRoll + 1934, rElbowPitch}; // angulos máximos (12Bits)/*/
...
*/
unsigned int minPos[MARTA_QTD] = {rHipYaw-341, rHipRoll-120, rHipPitch-455,rKneePitch-1024, rAnklePitch-455, rAnkleRoll-341,
                                   lHipYaw-341, lHipRoll-455, lHipPitch-1024, lKneePitch-1024, lAnklePitch-455, lAnkleRoll-341}; // angulos minimos (12Bits)
                                   //LIMITES MÍNIMOS ORIGINAIS*/

//unsigned int minPos[MARTA_QTD] = {rHipYaw-341, rHipRoll-455, rHipPitch-455,rKneePitch-1024, rAnklePitch-455, rAnkleRoll-500,
//                                   lHipYaw-341, lHipRoll-455, lHipPitch-1024, lKneePitch-1024, lAnklePitch-455, lAnkleRoll-500}; // angulos minimos (12Bits)
...
unsigned int maxPos[MARTA_QTD] = {rHipYaw+341, rHipRoll+455, rHipPitch+1024,rKneePitch+1024, rAnklePitch+455, rAnkleRoll+341, 
                                  lHipYaw+341, lHipRoll+120, lHipPitch+455,lKneePitch+1024, lAnklePitch+455, lAnkleRoll+341}; // angulos máximos (12Bits)
                                  //LIMITES MÁXIMOS ORIGINAIS*/
                                  
//unsigned int maxPos[MARTA_QTD] = {rHipYaw+341, rHipRoll+455, rHipPitch+1024,rKneePitch+1024, rAnklePitch+455, rAnkleRoll+500, 
//                                  lHipYaw+341, lHipRoll+455, lHipPitch+455,lKneePitch+1024, lAnklePitch+455, lAnkleRoll+500}; // angulos máximos (12Bits)                                
...
//LEGS
int32_t goal_position_R[QTD] = {2048, 2048-30, 2048, 2048, 2048, 2048};
...
//VARIABLES TO CONTROL SERIAL PORT VELOCITY
const int32_t VELOCITY_QTD = 4;
int32_t VELOCITY[VELOCITY_QTD] = {0, 0, 0, 0};
...
void arms_cb(const std_msgs::Int16MultiArray& cmd_msg){
  int i;
  int mult_arm; 
  int32_t goal_position_arm_teste[QTD_ARMS] = {1024, lSholderRoll, lElbowPitch, rSholderPitch, rSholderRoll, rElbowPitch};
  for(i = 0; i < QTD_ARMS; i++){
   mult_arm = reversed(joint_reversed_arms[i]);
   goal_position_arm[i] = constrained(arm_joints_initial[i] + mult_arm*convert(cmd_msg.data[i]), minPos_arm[i], maxPos_arm[i]);
   //goal_position_arm[i] = goal_position_arm_teste[i];
  }
  sprintf(str, "%d, %d, %d, %d, %d, %d", goal_position_arm[0], goal_position_arm[1], goal_position_arm[2], goal_position_arm[3], goal_position_arm[4], goal_position_arm[5]);
  msg.data = str;
  delay(1);
  deb.publish(&msg);
...
void velocity_cb(const std_msgs::Int16MultiArray& cmd_msg){
  int i;
  for(i = 0; i < VELOCITY_QTD; i++){
    VELOCITY[i] = cmd_msg.data[i]; 
  }
}
...
ros::Subscriber<std_msgs::Int16MultiArray> velocity("velocity", velocity_cb);
...
void loop() {
  ...
  dxl1.writeControlTableItem(MOVING_SPEED, BROADCAST_ID, VELOCITY[0]);
  dxl2.writeControlTableItem(MOVING_SPEED, BROADCAST_ID, VELOCITY[1]);
  dxl3.writeControlTableItem(MOVING_SPEED, BROADCAST_ID, VELOCITY[2]);
  dxl5.writeControlTableItem(MOVING_SPEED, BROADCAST_ID, VELOCITY[3]);
