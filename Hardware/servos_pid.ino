...
const uint8_t head1[DXL_ID_CNT1] = {180, 88, 180, 200, 202, 72, 150, 180, 180, 180, 170, 180 ,180, 180};
...
void servo_cb(const std_msgs::Int16MultiArray& cmd_msg) {
  ...
  for (int i = 0; i < DXL_ID_CNT; i++) {
    head[i] = head_initial[i] + cmd_msg.data[i];
...
void setup() {
  ...
  for (int i = 0; i < DXL_ID_CNT1; i++) {
    dxl.setGoalPosition(DXL_ID_LIST[i], head1[i], UNIT_DEGREE);
    delay(0.1);
  }

  // Set Position PID Gains
  for(int i = 0; i < DXL_ID_CNT; i++){
    dxl.writeControlTableItem(POSITION_P_GAIN, ID_HEAD[i], position_p_gain);
    dxl.writeControlTableItem(POSITION_I_GAIN, ID_HEAD[i], position_i_gain);
    dxl.writeControlTableItem(POSITION_D_GAIN, ID_HEAD[i], position_d_gain);
    //dxl.writeControlTableItem(PROFILE_VELOCITY, ID_HEAD[i], 0);
  }
  //Imu(bno) setup
...
void set_position(){
  for (int i = 0; i < DXL_ID_CNT; i++) {
    dxl.setGoalPosition(ID_HEAD[i], head[i], UNIT_DEGREE);
    delay(0.001);
  }
}
