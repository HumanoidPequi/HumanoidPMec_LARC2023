...
const int DXL_DIR_PIN1 = 12; // DYNAMIXEL Shield DIR PIN
const int DXL_DIR_PIN2 = 13; // DYNAMIXEL Shield DIR PIN
...
void set_position(){
  for (int i = 0; i < DXL_ID_CNT_LEGS; i++) {
    dxl1.setGoalPosition(ID_LEFT_LEG[i], legs[i], UNIT_DEGREE);
    delay(0.001);
  }
  for (int i = 7; i < DXL_ID_CNT_LEGS + 6; i++) {
    dxl2.setGoalPosition(ID_RIGHT_LEG[i], legs[i], UNIT_DEGREE);
    delay(0.001);
  }
}
...
void setup_dynamixel(){
  ...
  dxl1.ping(BROADCAST_ID);
  dxl1.torqueOff(BROADCAST_ID);
  dxl1.setOperatingMode(BROADCAST_ID, OP_POSITION);
  ...
  dxl1.writeControlTableItem(PROFILE_VELOCITY,BROADCAST_ID, 0);
  
  //Serial 2
  dxl2.setPortProtocolVersion(DXL_PROTOCOL_VERSION);

  
  dxl2.ping(BROADCAST_ID);
  dxl2.torqueOff(BROADCAST_ID);
  dxl2.setOperatingMode(BROADCAST_ID, OP_POSITION);
  dxl2.torqueOn(BROADCAST_ID);

  for (int i = 6; i < DXL_ID_CNT_LEGS + 6; i++) {
    dxl2.setGoalPosition(ID_RIGHT_LEG[i], legs[i], UNIT_DEGREE);
    delay(0.0001);
  }
  ...
  dxl2.writeControlTableItem(PROFILE_VELOCITY, BROADCAST_ID, 0);
